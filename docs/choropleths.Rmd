...
```{r}
states <- sf::sf_read("https://rstudio.github.io/leaflet/json/us-states.geojson")
class(states)
names(states)
```

As you can see, we now have a `sf` with `name` (state name) and `density` (population density in people/mi<sup>2</sup>) columns from the GeoJSON.
