...
methods.setStyle = function (category, layerId, style) {
  var map = this;
  if (!layerId) {
    return;
  } else if (!((typeof layerId === "undefined" ? "undefined" : _typeof(layerId)) === "object" && layerId.length)) {
    // in case a single layerid is given
    layerId = [layerId];
  }

  //convert columnstore to row store
  style = _htmlwidgets2.default.dataframeToD3(style);
  //console.log(style);

  layerId.forEach(function (d, i) {
    var layer = map.layerManager.getLayer(category, d);
    if (layer) {
      // or should this raise an error?
      layer.setStyle(style[i]);
    }
  });
};

methods.setRadius = function (layerId, radius) {
  var map = this;
  if (!layerId) {
    return;
  } else if (!((typeof layerId === "undefined" ? "undefined" : _typeof(layerId)) === "object" && layerId.length)) {
