...
# popup
m %>% popup(42.0285, -93.65, 'Here is the <b>Department of Statistics</b>, ISU')
rand_lat = function(n = 10) rnorm(n, 42.0285, .01)
rand_lng = function(n = 10) rnorm(n, -93.65, .01)

# TODO: only one popup can be opened; need to bind all popups in a layer and show all
m %>% popup(rand_lat(), rand_lng(), 'Random popups')

# marker
m %>% marker(rand_lat(), rand_lng())

# circle (units in metres)
m %>% circle(rand_lat(50), rand_lng(50), radius = runif(50, 5, 15))

# circle marker (units in pixels)
m %>% circleMarker(rand_lat(50), rand_lng(50), options = list(color = '#ff0000'))
m %>% circleMarker(rand_lat(100), rand_lng(100), radius = runif(100, 5, 15))
...
# polyline
m %>% polyline(rand_lat(50), rand_lng(50))

# polygon
m %>% polygon(rand_lat(), rand_lng(), layerId = 'foo')
...
m %>% setView(c(47.6759920119894, -122.36075812146), zoom = 13) %>% geoJSON(seattle_geojson)
