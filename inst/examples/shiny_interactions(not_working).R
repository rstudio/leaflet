      ...
      fluidPage(
        fluidRow(
          column(2,
            ...
            # for some reason appears to have a country selected
            selectInput('countries', label = "Make a country blue", choices = countries,selected=NULL)
          ),
          column(10,
             # Can't get map to show full-screen, see prod on old version
...
server <- function(input, output, session) {
  ...
  observeEvent(input$countries, {
    if(is.null(input$countries)==FALSE ){
      print("hey")
      leafletProxy("mymap") %>%
      setStyleGeoJSON(layerId ='geojsonlayer', attribute = "admin",
                      command = "=", value = input$countries, style = '{"fillColor" :"blue"}')
    }
  })
