...
For convenience leaflet provides a `provideres` list, pre-populated with all the third-party tile providers supported by the leaflet-providers plugin. This enables you to use auto-completion feature of your favorite R IDE (like RStudio) and not have to remember or look up supported tile  providers.
...
To see a list of all supported tile providers see `names(providers)`. You can pass additional parameters to the tile provider by populating the `options` argument using `providerTileOptions()` function. This can for example be used to pass in API access token/key required by certain providers.
