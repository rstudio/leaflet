...
Labels are elemets either text or HTML content that can be shown on markers, shapes either on hover or statically. Unlike popups you don't need to click a marker/polygon for the label to be shown.
...
You can customize maker labels using the `labelOptions` argument of the `addMarkers` function. The `labelOptions` argument can be populated using the `labelOptions()` function. If `noHide` is false (the default) then the label is displayed only when you hover the mouse over the marker, if `noHide` is set to true, then the label is displayed statically.
